...
// Console-only logger; the worker runtime forwards console.* to the iframe as SSE log events.
function createStepLogger(): {
  ...
  return { logger: makeLogger(), drain: () => Promise.resolve() };
}
...
export function createWorkflowFunction<TData, TPayload = TData, TResult = unknown>(
  ...
  return async ({
    ...
    for (let attempt = 0; ; attempt++) {
      const { logger: log, drain: drainLog } = createStepLogger();
      try {
