...
export function extractWrappedXml(text: string, wrapperTag: string): string | null {
  if (text.includes(`<${wrapperTag}>`)) return text;
  if (new RegExp(`<${wrapperTag}\\s*/>`).test(text)) return `<${wrapperTag} />`;
  return null;
}
