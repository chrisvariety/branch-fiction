...
/**
 * Build the hierarchy tree starting from a given entity, returning all descendants
 * (including the start entity itself).
 *
 * @param hierarchies - All hierarchy nodes for the book
 * @param startEntityId - The root entity ID to start from
 * @returns Array of entity IDs including the start entity and all its descendants
 */
...
/**
 * Build hierarchical path strings for places (e.g., "The Citadel > Great Hall > throne room")
 *
 * @param hierarchies - All hierarchy nodes for the book
 * @param placeEntityIds - Array of entity IDs to build paths for
 * @param entityIdToName - Map of entity ID to entity name
 * @returns Record mapping entity ID to its hierarchical path string
 */
...
export function processHierarchyData<T extends BaseEntity>(
  ...
  // 4. Count ERAs at each level using the Era Detector algorithm
  // Build a minimal name map (we don't need names for counting)
  ...
  // 5. Determine preferred level based on ERA counts
  // Prefer HUB if it has 2+ ERAs, otherwise try LOCALE if it has more ERAs
