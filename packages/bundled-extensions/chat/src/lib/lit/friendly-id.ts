...
/**
 * Generate a unique friendly prefix for a set of entities, handling collisions
 * by progressively expanding initials.
 *
...
export async function generateUniqueFriendlyPrefix({
  ...
  // Strategy 1: Try multi-word initials (first letter from each word)
  // Determine max number of words across all entity names
  ...
  // Final fallback: use last 4 chars of each entity ID for uniqueness
  // (This works better with UUIDv7 where prefixes are time-based and similar)
  // (shouldn't happen though as `slug` will return a short, predictable hash when the word(s) cannot be slugged)
...
/**
 * Extract initials from slugged words with configurable length.
 * Takes the first N characters from the first word.
 */
...
/**
 * Combine initials from multiple entity word arrays.
 * Uses single-word expansion strategy (first N chars from first word).
 */
...
/**
 * Combine multi-word initials from multiple entity word arrays.
 * Uses multi-word strategy (first letter from each of first N words).
 */
