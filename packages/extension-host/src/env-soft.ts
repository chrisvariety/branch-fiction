// Third-party libs probe and mutate env vars at import/init time
// (`@google/genai` checks GOOGLE_SDK_NODE_LOGGING, `debug` deletes
// process.env.DEBUG, etc.). Extension workers run with no `--allow-env`, so
// each call throws `NotCapable` and breaks module load. Patch `Deno.env`
// to swallow those throws.
