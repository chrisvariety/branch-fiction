...
// Reroute console to stderr so stdout stays clean for JSON-RPC frames.
for (const k of ['log', 'info', 'warn', 'error', 'debug', 'trace'] as const) {
  console[k] = (...args: unknown[]) => writeStderr(fmtArgs(args));
}

// Notifications carry async events from the worker to the host (e.g.
// `host.log(...)`). They have no `id` and never get a response.
...
export async function serveRPC(api: API): Promise<never> {
  writeStderr('[extension-host] worker ready');
...
async function handleLine(line: string, api: API): Promise<void> {
  let req: Request;
  try {
    req = JSON.parse(line);
  } catch (e) {
